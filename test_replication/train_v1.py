...
# Import load_file from data_utils for validation data
from data_utils import load_file as load_file_full
...
MAX_TRAIN_LINES = 99998  # Limit training data to first 99,998 lines
...
if __name__ == '__main__':
    ...
    save_path = os.path.join('outputs', f'model_v1_{timestamp}')
    ...
    print("=" * 60)
    print("Chinese Punctuation Restoration - Training (V1)")
    print("=" * 60)
    ...
    # Load data (limited to first 99,998 lines)
    print("\nLoading data (limited to first 99,998 lines)...")
    data_train = load_file_limited('data/train', max_lines=MAX_TRAIN_LINES)
    data_valid = load_file_full('data/valid')
    ...
    criterion = nn.CrossEntropyLoss() # fixme: in train_v2.py for F1 Scores: calculate class weights to handle imbalanced data, count class frequencies compute from data)

    # class_weights = torch.tensor([
    #     1.0,  # O (most common - weight 1.0)
    #     10.0,  # ， (comma - 10x weight)
    #     10.0,  # 。 (period - 10x weight)
    #     20.0,  # ？ (question - 20x weight, rarer)
    #     20.0,  # ！ (exclamation - 20x weight, rarer)
    #     15.0,  # ； (semicolon - 15x weight)
    #     15.0,  # 、 (enumeration - 15x weight)
